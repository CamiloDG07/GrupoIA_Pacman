...
Todos los comandos se corren desde una terminal de VS Code. Hay dos tipos
de comando por actividad:

- **Comando de datos**: `python searchAgents.py N`, donde `N` es el número
  de actividad (1 a 11). Se corre **desde dentro de `pacman/`**
  (`cd pacman` primero). Imprime texto con los números (nodos expandidos,
  costo, tiempo) y los guarda en un `resultados.csv` en el directorio desde
  el que se corre. La lógica vive al final de `pacman/searchAgents.py`
  (funciones `demo_actividadN`) — uno de los 4 archivos del zip de
  entrega —, así que este mismo comando funciona igual si el profesor
  evalúa poniendo solo `search.py`, `searchAgents.py`, `resultados.csv` e
  `informe.pdf` (el contenido de `Grupo04_Pacman_AStar.zip`) sobre su
  propio proyecto base (`pacman.py`, `layout.py`, `game.py`, `util.py`,
  `layouts/`, etc.), sin el resto del repositorio. Sin argumentos
  (`python searchAgents.py`) corre las 11 actividades en orden; con varios
  números (`python searchAgents.py 7 8 9`) corre solo esas.
...
- **Datos:** `cd pacman && python searchAgents.py 1`
...
- **Datos:** `cd pacman && python searchAgents.py 4`
...
- **Datos:** `cd pacman && python searchAgents.py 5`
...
- **Datos:** `cd pacman && python searchAgents.py 6`
...
- **Datos:** `cd pacman && python searchAgents.py 7`
...
- **Datos:** `cd pacman && python searchAgents.py 8`
...
- **Datos:** `cd pacman && python searchAgents.py 9`
...
- **Datos:** `cd pacman && python searchAgents.py 10`
...
- **Datos:** `cd pacman && python searchAgents.py 11`
